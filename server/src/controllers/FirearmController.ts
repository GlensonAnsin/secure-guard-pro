...
class FirearmController {
  ...
  public async show(req: Request, res: Response, next: NextFunction) {
    try {
      const firearm = await FirearmService.getFirearmById(Number(req.params.id));
      if (!firearm) return ApiResponse.error(res, 'Firearm not found', 404);
      return ApiResponse.success(res, firearm, 'Firearm retrieved successfully');
    } catch (error) {
      next(error);
    }
  }
  ...
  public async store(req: Request, res: Response, next: NextFunction) {
    try {
      const firearm = await FirearmService.createFirearm(req.body);
      return ApiResponse.success(res, firearm, 'Firearm created successfully', 201);
    } catch (error) {
      next(error);
    }
  }
  ...
  public async update(req: Request, res: Response, next: NextFunction) {
    try {
      const firearm = await FirearmService.updateFirearm(Number(req.params.id), req.body);
  ...
  public async destroy(req: Request, res: Response, next: NextFunction) {
    try {
      await FirearmService.deleteFirearm(Number(req.params.id));
...
export default new FirearmController();
